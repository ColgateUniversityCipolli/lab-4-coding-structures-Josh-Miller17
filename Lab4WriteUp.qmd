...
for(i in 1:nrow(df)){
  df$total.score[i] <- calculate.score(df$x1[i], df$x3[i], df$x3[i]) }
```
...
```{r}
#| size: "scriptsize"
html <- read_html("website/KimchiGrilledCheese.html")
```
...
```{r}
#| size: "scriptsize"
ingredients.section <- html_element(html, "#ingredients")
```
...
```{r}
#| size: "scriptsize"
instructions.section <- html_element(html, "#instructions")
```
...
```{r}
#| size: "scriptsize"
image.element <- html_element(html, ".quarto-figure")
```
