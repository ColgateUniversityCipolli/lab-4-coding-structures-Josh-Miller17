...
# create a for loop which takes the  image, ingredients, and instructions for each selected recipe. Used last doc as a reference.

# Combined all of the ingredients into a grocery list on the first page.

# Print the image, ingredients, and instructions for each receipe on the subsequent pages
```
...
chosen.recepies <- c(chosen.breakfast, chosen.dinners) #created vector with all chosen breakfasts and dinners.
```
